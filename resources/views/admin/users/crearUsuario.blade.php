...
<div class="container">
   ...
   <div class="card-body">
      <form method="POST" action="{{ route("admin.guardarUsuario") }}">
         {{-- {{ method_field('PUT') }} --}}
         {!! csrf_field() !!}
         <div class="form-group{{ $errors->has('pseudonombre') ? ' has-error' : '' }}">
            <label for="pseudonombre">Alias</label>
            <input type="text" name="pseudonombre" class="form-control" placeholder="Pseudonombre" value="{{ old('pseudonombre') }}">
         ...
         <div class="form-group{{ $errors->has('nombres') ? ' has-error' : '' }}">
            ...
            <input type="text" name="nombres" class="form-control" placeholder="Nombre(s)" value="{{ old('nombres') }}">
         ...
         <div class="form-group{{ $errors->has('app') ? ' has-error' : '' }}">
            ...
            <input type="text" name="app" class="form-control" placeholder="Apellido Paterno" value="{{ old('app') }}">
            @if ($errors->has('app'))
             <span class="help-block">
         ...
         <div class="form-group{{ $errors->has('apm') ? ' has-error' : '' }}">
            ...
            <input type="text" name="apm" class="form-control" placeholder="Apellido Materno" value="{{ old('apm') }}">
            @if ($errors->has('apm'))
             <span class="help-block">
         ...
         <div class="form-group">
            ...
            <input type="checkbox" {{ old('activo') ? 'Checked' : '' }} class="filled-in form-check-input" name="activo" id="activo" value="1">
         </div>
         ...
         <div class="form-group{{ $errors->has('fechaNac') ? ' has-error' : '' }}">
            ...
            <input type="date" name="fechaNac" id="fechaNac" class="form-control" value="{{ old('fechaNac') }}" autocomplete="off" @if ($errors->has('fechaNac')) autofocus @endif>
         ...
         <div class="form-group{{ $errors->has('telFijo') ? ' has-error' : '' }}">
            ...
            <input type="text" name="telFijo" id="telFijo" class="form-control" placeholder="Incluye Lada" value="{{ old('telFijo') }}" @if ($errors->has('telFijo')) autofocus @endif autocomplete="off" maxlength="10">
         ...
         <div class="form-group{{ $errors->has('telCelular') ? ' has-error' : '' }}">
            ...
            <input type="text" name="telCelular" id="telCelular" class="form-control" placeholder="Incluye Lada" value="{{ old('telCelular') }}" @if ($errors->has('telCelular')) autofocus @endif autocomplete="off" maxlength="13">
         ...
         <div class="form-group{{ $errors->has('rol') ? ' has-error' : '' }}">
            ...
            <select name="rol" id="rol" class="form-control" autocomplete="off" @if ($errors->has('rol')) autofocus @endif>
               @foreach ($roles as $rol)
                  <option title="rol" value="{{ $rol->id }}" {{ old('rol') ? 'selected':'' }} >{{ $rol->nombre }}</option>
               @endforeach
            </select>
         </div>
         <div class="form-group{{ $errors->has('puesto') ? ' has-error' : '' }}">
            ...
            <select name="puesto" id="puesto" class="form-control" @if ($errors->has('puesto')) autofocus @endif>
               @foreach ($puestos as $puesto)
                  <option title="puesto" value="{{ $puesto->id }}" {{ old('puesto') ? 'selected':'' }} >
                     {{ $puesto->nombre }}
                  </option>
               @endforeach
            </select>
         ...
         <div class="form-group{{ $errors->has('area') ? ' has-error' : '' }}">
            ...
            <select name="area" id="area" class="form-control" @if ($errors->has('area')) autofocus @endif>
               @foreach ($areas as $area)
                  <option title="area" value="{{ $area->id }}" {{ old('area') ? 'selected':'' }} >
                     {{ $area->nombre }}
                  </option>
               @endforeach
            </select>
